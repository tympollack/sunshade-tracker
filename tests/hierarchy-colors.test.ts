...
describe('Hierarchy Level Colors Utility', () => {
  ...
  it('maps level 1 to purple palette', () => {
    ...
    expect(color.hex).toBe('#c084fc');
    expect(color.badgeBg).toBe('bg-slate-900');
    expect(color.badgeText).toBe('text-purple-300');
  });
  ...
  it('maps level 2 to sky palette', () => {
    ...
    expect(color.badgeBg).toBe('bg-slate-900');
    expect(color.badgeText).toBe('text-sky-300');
  });

  it('maps level 3 to emerald palette', () => {
    const color = getHierarchyLevelColor('story', sampleHierarchy);
    expect(color.hex).toBe('#34d399');
    expect(color.badgeBg).toBe('bg-slate-900');
    expect(color.badgeText).toBe('text-emerald-300');
  });
  ...
  it('maps level 4 to amber palette', () => {
    ...
    expect(color.badgeBg).toBe('bg-slate-900');
    expect(color.badgeText).toBe('text-amber-300');
  });

  it('maps level 5 to rose palette', () => {
    const color = getHierarchyLevelColor('subtask', sampleHierarchy);
    expect(color.hex).toBe('#fb7185');
    expect(color.badgeBg).toBe('bg-slate-900');
    expect(color.badgeText).toBe('text-rose-300');
  });
  ...
  it('provides getDefaultLevelHex helper', () => {
    expect(getDefaultLevelHex(1)).toBe('#c084fc');
    expect(getDefaultLevelHex(2)).toBe('#38bdf8');
    expect(getDefaultLevelHex(3)).toBe('#34d399');
    expect(getDefaultLevelHex(4)).toBe('#fbbf24');
    expect(getDefaultLevelHex(5)).toBe('#fb7185');
    expect(getDefaultLevelHex(99)).toBe('#94a3b8');
  });

  it('provides sensible fallback for unknown type without hierarchy config', () => {
    const color = getHierarchyLevelColor('epic', []);
    expect(color.hex).toBe('#c084fc'); // Fallback heuristic maps epic to level 1
